...
class ProcesarXmlF15Command extends ContainerAwareCommand{

    protected function configure (){
        $this -> setName('analisis:procesaXmlF15');
        $this -> setDescription('Procesa un documento XML Feed15');
        ...
        foreach ($iterator as $file) {
          if($file->getFilename() == ".DS_Store")continue;
            if(preg_match("/F15$/", $file->getPath())){
                  ...
                  foreach ($xml->children() as $soccedocument) {
                      $atributos = $soccedocument->attributes();
